...
* Every time after the user quits the app, we don't really want them to remember what they last typed in to search for the purchase. Apply what you've learned about user shared preferences from the link below to add this functionality to the app so that when the user restarts the app, the search bar comes prefilled with the last description they previously used to search.
